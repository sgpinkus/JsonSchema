...
class JsonSchemaTest extends PHPUnit_Framework_TestCase
{
  ...
  public function testJsonSchema() {
    ...
    $schemaDoc = $jsonDocs->loadDocStr(file_get_contents(getenv('DATADIR') . '/user-schema.json'), new Uri("file:///user-schema.json"));
    $targetDoc = $jsonDocs->loadDocStr(file_get_contents(getenv('DATADIR') . '/user.json'), new Uri("file:///user.json"));
